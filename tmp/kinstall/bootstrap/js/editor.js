...
function kGenerateColorPalette(width, height)
{
	...
	html +='<table id="kbbcode-colortable" class="kbbcode-colortable" cellspacing="1" cellpadding="0" border="0" style="width: 100%;">';
	...
	for (r = 0; r < 5; r++)
	{
		...
		for (g = 0; g < 5; g++)	{
			for (b = 0; b < 5; b++)	{
				...
				html +='<td style="background-color:#' + color + '; width: ' + width + '; height: ' + height + ';">&nbsp;</td>';
			  }
		}
		html +='</tr>';
	}
	html +='</table>';
	jQuery('#kbbcode-color-options').html(html);
}
