...
<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" class="postform form-validate" id="postform" name="postform" enctype="multipart/form-data">
...
<div class="kblock">
	...
	<div class="kcontainer">
		<div class="kbody">
<table class="table table-striped<?php echo !empty ( $this->category->class_sfx ) ? ' table table-striped' . $this->escape($this->category->class_sfx) : ''?>" id="kpostmessage">
	<tbody id="kpost-message">
		...
		<tr id="kpost-topicicons" class="krow<?php echo 1 + $this->k^=1 ?>">
			...
			<td class="kcol-mid">
				...
				<input type="radio" class="kiconsel" name="topic_emoticon" id="topic_emoticon_<?php echo $icon->id ?>" value="<?php echo $icon->id ?>" <?php echo !empty($icon->checked) ? ' checked="checked" ':'' ?> />
				<label class="kiconsel" for="topic_emoticon_<?php echo $icon->id ?>">
		...
		<tr id="kpost-attachments" class="krow<?php echo 1 + $this->k^=1;?>">
			...
			<td class="kcol-mid">
				<div id="kattachment-id" class="kattachment">
					<div class="kattachment-id-container">

					<div class="kfile-input-textbox pull-left label"></div>
					<div class="editlinktip hide-kupload pull-left" title="<?php echo JText::_('COM_KUNENA_FILE_EXTENSIONS_ALLOWED')?>::<?php echo $this->escape(implode(', ', $this->allowedExtensions)) ?>" >
						<span class="kfile-input-button kbutton"><span class="icon icon-upload"></span> <?php echo  JText::_('COM_KUNENA_EDITOR_ADD_FILE'); ?></span>
						<input class="kfile-input" name="kattachment" type="file" />
					</div>
