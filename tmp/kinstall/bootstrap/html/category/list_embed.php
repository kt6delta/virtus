...
foreach ( $this->sections as $section ) :
	$htmlClassBlockTable = !empty ( $section->class_sfx ) ?  $this->escape($section->class_sfx) : '';
...
<div class="kblock kcategories-<?php echo intval($section->id) ?>">
	<div class="kheader">
		...
		<span class="ktoggler"><a class="ktoggler icon icon-minus-circle close" title="<?php echo JText::_('COM_KUNENA_TOGGLER_COLLAPSE') ?>" data-toggle="collapse" data-target="#catid_<?php echo intval($section->id) ?>"></a></span>
		<?php endif; ?>
	...
	<div class="kcontainer  collapse in" id="catid_<?php echo intval($section->id) ?>">
		<div class="kbody">
<table class="table <?php echo $htmlClassBlockTable ?>" id="kcat<?php echo intval($section->id) ?>">
	...
	?>
		<tr class="k<?php echo $tabclass [$k ^= 1], isset ( $category->class_sfx ) ? ' k' . $this->escape($tabclass [$k]) . $this->escape($category->class_sfx) : '' ?>"
			...
			<?php
				...
				if ($category->locked) {
					echo $this->getIcon ( 'icon-locked', JText::_('COM_KUNENA_LOCKED_CATEGORY') );
				}
				if ($category->review) {
					echo $this->getIcon ( 'icon-user', JText::_('COM_KUNENA_GEN_MODERATED') );
				}
				?>
			</h4>
