			...
			<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" id="adminForm" name="adminForm">
				...
				<table class="table table-striped">
					<tr class="ksth">
						...
						<th class="hidden-phone"><?php echo JText::_('COM_KUNENA_FILETYPE'); ?></th>
					...
					<tr class="k<?php echo $usrl_class ;?>">
						<td class="kcol-first hidden-phone"><?php echo $y; ?></td>
						<td class="kcol-mid"><?php if ($item->authorise('delete')) echo JHtml::_('grid.id', $i, intval($item->id)) ?></td>
						<td class="text-center hidden-phone kcol-mid"><img src="<?php echo $item->filetype != '' ? JUri::root(true).'/media/kunena/icons/image.png' : JUri::root(true).'/media/kunena/icons/file.png'; ?>" alt="" title="" /></td>
						...
						<td align="center" class="kcol-mid atachements-preview"><?php echo $item->getThumbnailLink() ; ?></td>
						...
						<td align="center" class="kcol-mid">
							...
							<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i; ?>','delfile')">
								<img src="<?php echo $this->ktemplate->getImagePath('remove.png') ?>" alt=""/>
							</a>
							<?php endif ?>
						</td>
				...
				<input class="kbutton pull-right" type="submit" value="<?php echo JText::_('COM_KUNENA_FILES_DELETE') ?>" />
			</form>
		</div>
	</div>
</div>
