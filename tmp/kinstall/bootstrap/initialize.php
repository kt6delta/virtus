...
if ( KunenaFactory::getConfig()->lightbox == 1 ) {
	$template->addStyleSheet ( 'gallery/css/lightGallery.css');
	$template->addScript( 'gallery/js/lightGallery.js' );

}

$template->addScript ( 'js/bootstrap-tooltip.js' );
$template->addScript ( 'js/kbootstrap.js' );
...
if (file_exists ( JPATH_ROOT . "/templates/".$tmplName."/css/kunena.forum.css" )) {
	// Load css from Joomla template
	CKunenaTools::addStyleSheet ( JUri::root(true). "/templates/".$tmplName."/css/kunena.forum.css" );
	if ($skinner && file_exists ( JPATH_ROOT. "/templates/".$tmplName."/css/kunena.skinner.css" )){
		CKunenaTools::addStyleSheet ( JUri::root(true). "/templates/".$tmplName."/css/kunena.skinner.css" );
	} elseif (!$skinner && file_exists ( JPATH_ROOT. "/templates/".$tmplName."/css/kunena.default.css" )) {
		CKunenaTools::addStyleSheet ( JUri::root(true). "/templates/".$tmplName."/css/kunena.default.css" );
	}
}


$cssurl = JUri::root(true) . '/components/com_kunena/template/bootstrap/css';

?>
	<link rel="stylesheet" href="<?php echo $cssurl; ?>/editor-icons.css" type="text/css"/>
	<!--[if lt IE 8]>
	<link rel="stylesheet" href="<?php echo $cssurl; ?>/ie7/ie7.css">
	<![endif]-->
<?php
